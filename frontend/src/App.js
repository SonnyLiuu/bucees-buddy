...
const App = () => {
  ...
  <>
    <Router>
      {user && <MainNavigation />}
      <Routes>
        <Route path="/" exact element={user ? <Stats /> : <Navigate to="/login"/>}/>
        <Route path="/trips" exact element={user ? <Trips /> : <Navigate to="/"/>}/>
        <Route path="/add" exact element={user ? <AddTrip /> : <Navigate to="/"/>}/>

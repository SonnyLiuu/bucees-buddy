...
const Map = (props) => {
    ...
    useEffect(() => {
        ...
        const colorlogo = 'https://cdn.discordapp.com/attachments/1125893813727154317/1133105429543669820/colorlogo.png';
        const graylogo = 'https://cdn.discordapp.com/attachments/1125893813727154317/1133105555678953503/graylogo.png';
        ...
        if (multipleMarkers === true)
        {
                ...
                {
                    for (let i = 1; i <= 58; i++){
                        if(i === 4 || i === 5 || i === 6 || i === 9 || i === 10 || i === 11 || i === 15 || i === 27 || i === 41 || i === 49 || i === 54 || i === 56)
                            continue;
                        ...
                        if (visitedFlag == true)
                            new window.google.maps.Marker({ position: {lat: coords[i].lat, lng: coords[i].lng}, map: map, icon: colorlogo});
                        else
                            new window.google.maps.Marker({ position: {lat: coords[i].lat, lng: coords[i].lng}, map: map, icon: graylogo});
                    }
                }
                else
                {
        ...
        else
        {
            if (latitude)
            {
                ...
                const newMap = new window.google.maps.Map(mapRef.current, {center: {lat: latitude,lng: longitude}, zoom: zoom});

                new window.google.maps.Marker({ position: {lat: latitude, lng: longitude}, map: newMap, icon: colorlogo});
            }
        }
    }, [Trips, longitude]);

...
router.delete('/:id', async (req, res) => {
  ...
  router.get("/:email", async (req, res) => {
	try {
	  ...
	  const userTrips = await Trip.find({email});
  
	  if (!userTrips || userTrips.length === 0) {
		return res.status(404).send({ message: "No trips found for the user." });
	  }
  
	  res.status(200).json({ data: userTrips, message: "User's Trips Retrieved Successfully!" });
	} catch (error) {
	  console.log(error);
...
router.post("/", async (req, res) => {
	// adding coordinates for a location
	let storeNumber = req.body.location.split(" ")[0];
	console.log(storeNumber);
	let coords = await Coordinates.findOne({storeNumber});
 ...
 let user = await User.findOne({ email: req.body.email });
  ...
  if(al_pattern.test(req.body.location)) {
    ...
    var trips = user.al_trips + 1;
		newStateTotal = trip.total + user.al_total;
		await User.updateOne({_id: user._id}, {al_total : newStateTotal});
	...
	newStateTotal = trip.total + user.fl_total;
	await User.updateOne({_id: user._id}, {fl_total : newStateTotal});
	...
	newStateTotal = trip.total + user.ga_total;
	await User.updateOne({_id: user._id}, {ga_total : newStateTotal});
  ...
  // Treating KY and SC like the multi-location states in case they get new ones in the future
	...
	newStateTotal = trip.total + user.ky_total;
	await User.updateOne({_id: user._id}, {ky_total : newStateTotal});
	...
	newStateTotal = trip.total + user.sc_total;
	await User.updateOne({_id: user._id}, {sc_total : newStateTotal});
	...
	newStateTotal = trip.total + user.tn_total;
	await User.updateOne({_id: user._id}, {tn_total : newStateTotal});
	...
	newStateTotal = trip.total + user.tx_total;
	await User.updateOne({_id: user._id}, {tx_total : newStateTotal});
  ...
  // Now that we have the specific store, we update that store's total trips and total spent
	...
	if(req.body.didHotGrab == true){
		...
		await User.updateOne({_id: user._id}, {total_hotgrab : new_total_hotgrab});
	...
	//new_loc_total is the updated amount of times we've visited this particular location. If it's 1, then this is our first visit.
	//Just make a 0/1 flag based on new_loc_total and add it to state_unique
	
	let newLocationFlag = 0;
	if(new_loc_total === 1)
		newLocationFlag = 1;
	
	let originalStateUnique = 0;
	
	if(state == "AL"){
		originalStateUnique = user.al_unique || 0;
		originalStateUnique += newLocationFlag;
		await User.updateOne({_id: user._id}, {al_unique : originalStateUnique});
	}

	else if(state == "FL"){
		originalStateUnique = user.fl_unique || 0;
		originalStateUnique += newLocationFlag;
		await User.updateOne({_id: user._id}, {fl_unique : originalStateUnique});
	}

	else if(state == "GA"){
		originalStateUnique = user.ga_unique || 0;
		originalStateUnique += newLocationFlag;
		await User.updateOne({_id: user._id}, {ga_unique : originalStateUnique});
	}

	else if(state == "KY"){
		await User.updateOne({_id: user._id}, {ky_unique : 1});
	}

	else if(state == "SC"){
		await User.updateOne({_id: user._id}, {sc_unique : 1});
	}

	else if(state == "TN"){
		originalStateUnique = user.tn_unique || 0;
		originalStateUnique += newLocationFlag;
		await User.updateOne({_id: user._id}, {tn_unique : originalStateUnique});
	}

	else if(state == "TX"){
		originalStateUnique = user.tx_unique || 0;
		originalStateUnique += newLocationFlag;
		await User.updateOne({_id: user._id}, {tx_unique : originalStateUnique});
	}
